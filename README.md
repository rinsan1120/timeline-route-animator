...
通常編集画面ではDAYごとにルートを固定色で表示します。色は10色のパレットを循環し、DAY11はDAY1と同じ色へ戻ります。動画ではStep 03の「DAYごとにルートを色分け」でプレビュー・MP4の色分けを切り替えられ、ONの場合は走行距離HUDの各DAY行にもルートと同じ色を表示します。
...
DAYマーカーの寸法は、現在のブラウザ表示（最小幅138px、DAY文字14px等）を基準に、地図viewportの`popupDisplayScale()`で1920×1080へ換算します。DOMとMP4は共通レイアウトを使用し、日付・補足の有無や文字幅を反映します。保存済みの配置offsetの意味は変更しません。

Step 03「動画にする」の「走行距離表示」をONにすると、地図とMP4にDAY別・TOTALの概算距離HUDを表示します（初期OFF）。編集画面では計画全体の最終距離、プレビューと動画では現在地点までの距離をkm小数1桁で表示します。アニメ範囲の途中開始では、それ以前の走行距離も含みます。DAY間の接続区間やカメラ移動だけの間は加算しません。
