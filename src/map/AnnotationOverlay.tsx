...
export default function AnnotationOverlay({ map, points, animationPoints, editMode, previewProgress }: AnnotationOverlayProps) {
  ...
  useLayoutEffect(() => {
    ...
    const positionBalloons = () => {
      ...
      visible.forEach((point, index) => {
        ...
        const below = projected.y - balloonHeight - 20 < 8;
        const top = Math.max(8, Math.min(height - balloonHeight - 8, below ? projected.y + 20 : projected.y - balloonHeight - 20));
        element.style.left = `${left}px`;
        element.style.top = `${top}px`;
        element.style.setProperty('--pointer-left', `${Math.max(10, Math.min(balloonWidth - 10, projected.x - left))}px`);
  ...
  return <div ref={containerRef} className="annotation-overlay">
